...
UA = (
    ...
    "Chrome/126.0 Safari/537.36 PatBot/0.2"
)
...
def _player_id(record: dict) -> str:
    ...
    value = record.get("player_id") or record.get("id") or player.get("player_id") or player.get("id")
...
def _player_meta(record: dict) -> dict:
    ...
    full_name = player.get("full_name") or player.get("name") or record.get("name") or f"{first} {last}".strip()
...
def fetch_position_projections(session, season, position, week=None, order_by="pts_ppr") -> list[dict]:
    params = [("season_type", "regular"), ("position[]", position), ("order_by", order_by)]
    data = _get_json(session, _projection_url(season, week), params=params)
    if not isinstance(data, list):
        raise SleeperDataError(f"Unexpected Sleeper projection response for {position}: {type(data).__name__}")
    return data
...
def derive_byes(session: requests.Session, season: int) -> dict[str, int]:
    ...
    try:
        ...
        all_teams = {"ARI","ATL","BAL","BUF","CAR","CHI","CIN","CLE","DAL","DEN","DET","GB","HOU","IND","JAX","KC","LV","LAC","LAR","MIA","MIN","NE","NO","NYG","NYJ","PHI","PIT","SEA","SF","TB","TEN","WAS"}
        seen_by_week = {}
        for game in games:
...
def build_live_dataframe(config: dict) -> tuple[pd.DataFrame, dict]:
    ...
    for position in POSITIONS:
        season_records = fetch_position_projections(session, season, position, week=None, order_by="pts_ppr")
        total_source_records += len(season_records)
        adp_records = fetch_position_projections(session, season, position, week=1, order_by="adp_dd_ppr")
        ...
        for record in season_records:
            ...
            if not name:
                continue
            scored = score_season_projection(stats, scoring=scoring, bonus_model=bonus_model, position=pos)
    ...
    df = df.sort_values(["player_id", "proj_points"], ascending=[True, False]).drop_duplicates(subset=["player_id"], keep="first")
    pos_limits = {"QB": 40, "RB": 100, "WR": 120, "TE": 50, "K": 32, "DEF": 32}
    kept = []
    for pos, limit in pos_limits.items():
        group = df[df["pos"] == pos].copy()
        group = group.sort_values(["adp", "proj_points"], ascending=[True, False]).head(limit)
        kept.append(group)
    ...
    df = df.sort_values(["adp", "proj_points"], ascending=[True, False]).reset_index(drop=True)
    ...
    meta = {
        ...
        "bonus_method": "Expected per-game threshold bonuses estimated from projected YPG using a normal-distribution approximation.",
...
def refresh_snapshot(
    ...
    try:
        ...
        meta["market_sources"] = {"pipeline": {"ok": False, "error": f"{type(exc).__name__}: {exc}"}}
