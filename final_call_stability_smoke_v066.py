...
from patbot.final_call import final_call_settings, run_final_call
...
def main():
    ...
    settings = final_call_settings(cfg)
    ...
    print(f"\nPatBot v{__version__} Final Call stability-gate smoke")
    print("Calibration state: Gibbs 1.01, Bijan 1.02, PatBot on the clock at 1.03.")
    print(
        f"A confirmed challenger now needs +{settings['overturn_required_margin']:.1f} lineup points "
        "to overturn the base score leader.\n"
    )
    print("Base score board top 4:")
    print(board[["name", "pos", "score", "proj_points", "vorp", "adp"]].head(4).to_string(index=False))
    ...
    print("\n=== STABILITY-GATED FINAL CALL ===\n")
    print(f"Recommendation:  {result.get('recommendation')}")
    print(f"Base leader:     {result.get('base_winner')}")
    print(f"Room-sim leader: {result.get('sim_winner')}")
    print(f"Stage:           {result.get('stage')}")
    print(f"Runs:            {result.get('runs')}")
    print(f"Horizon:         Round {result.get('through_round')}")
    print(f"Sim-leader edge: {result.get('edge')} ({result.get('edge_label')})")
    ...
    print("\nSMOKE: Stability-gated Final Call completed normally.\n")
